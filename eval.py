...
import psutil
import torch
from dp import BatchGraph, SimpleBatchTopK, StreamingTopK, run_dp
from problems import load_problem
from torch.utils.data import DataLoader, Dataset
from tqdm import tqdm
from utils.data_utils import load_heatmaps, save_dataset
from utils.functions import accurate_cdist, compute_batch_costs, get_durations, move_to
...
def evaluate_dp(
    ...
    coords = (
        torch.cat((batch["depot"][:, None], batch["loc"]), 1).float()
        if is_vrp or has_tw
        else batch
    )
    ...
    graph = BatchGraph.get_graph(
        ...
        knn=knn,
        quantize_cost_dtype=quant_c_dt,
        demand=demands,
    ...
    if False:
        ...
        candidate_queue = StreamingTopK(
            ...
            payload_dtypes=(
                torch.int32,
                torch.int16,
            ),  # parent = max 1e9, action = max 2e3 (for VRP with 1000 nodes)
            device=coords.device,
            alloc_size_factor=10.0
            if beam_size * graph.batch_size <= int(1e6)
            else 2.0,  # up to 1M we can easily allocate 10x so 10MB
    ...
    mincost_dp_qt, solution = run_dp(
        ...
        beam_device=coords.device,
        bound_first=True,  # Always bound first #is_vrp or beam_size >= int(1e7),
        sort_beam_by="group_idx",
        trace_device="cpu",
        verbose=verbose,
        add_potentials=add_potentials,
    )
...
class HeatmapDataset(Dataset):
    def __init__(self, dataset=None, heatmaps=None):
        ...
        assert len(self.dataset) == len(
            self.heatmaps
        ), f"Found {len(self.dataset)} instances but {len(self.heatmaps)} heatmaps"

    def __getitem__(self, item):
        return {"data": self.dataset[item], "heatmap": self.heatmaps[item]}
...
def pack_heatmaps(dataset, opts, offset=None):
    ...
    return HeatmapDataset(
        dataset,
        load_heatmaps(opts.heatmap, symmetric=opts.problem != "tsptw")[
            offset : offset + len(dataset)
        ],
    )
...
def eval_dataset_mp(args):
    ...
    make_dataset_kwargs = (
        {"normalize": False}
        if opts.decode_strategy[:4] in ("dpbs", "dpdp") and problem.NAME == "cvrp"
        else {}
    )
    ...
    device = torch.device(
        "cuda:{}".format(device_num) if device_num is not None else "cpu"
    )
    ...
    return _eval_dataset(
        ...
        opts,
        device,
        no_progress_bar=opts.no_progress_bar or i > 0,
    )  # Disable for other processes
...
def eval_dataset(dataset_path, beam_size, opts):
    ...
    opts.system_info = {
        ...
        "devices": ["cpu"]
        if not use_cuda
        else [torch.cuda.get_device_name(i) for i in range(device_count)],
    ...
    if num_processes > 1:
        ...
        assert (
            opts.val_size % num_processes == 0
        ), f"Dataset size {opts.val_size} must be divisible by {device_count} devices x {opts.num_processes} processes = {num_processes}"
                    ...
                    pool.map(
                        ...
                        [
                            (
                                ...
                                opts,
                                i,
                                i % device_count if use_cuda else None,
                                num_processes,
                            )
    ...
    heatmap_basename = (
        os.path.splitext(os.path.split(opts.heatmap)[-1])[0]
    ...
    if opts.o is None:
        results_dir = os.path.join(
            opts.results_dir,
            "vrp" if problem.NAME == "cvrp" else problem.NAME,
            dataset_basename,
        )
        ...
        out_file = os.path.join(
            ...
            "{}{}{}-{}-{}{}{}-{}{}{}".format(
                ...
                beam_size,
                opts.score_function,
                "th" + str(opts.heatmap_threshold)
                if opts.heatmap_threshold is not None
                else "",
...
def print_statistics(results, opts):
    ...
    results_stat = [
        (cost, tour, duration) for (cost, tour, duration) in results if tour is not None
    ]
    if len(results_stat) < len(results):
        failed = [
            i + opts.offset
            for i, (cost, tour, duration) in enumerate(results)
            if tour is None
        ]
        print("*" * 100)
        print(
    ...
    (
        ...
        total_duration_parallel,
        total_duration_single_device,
        effective_batch_size,
    ) = get_durations(durations, batch_size, num_processes, device_count)

    print(
        "Average serial duration (per process per device): {:.3f}".format(
            avg_serial_duration
        )
    )
    if batch_size > 1:
        print(
            "Average parallel duration (per process per device), effective batch size {:.2f}): {:.3f}".format(
    ...
    print(
        "Calculated total duration for {} instances with {} processes on 1 device in parallel: {}".format(
    ...
    for batch in tqdm(dataloader, disable=no_progress_bar):
        ...
        with torch.no_grad():
            if opts.decode_strategy[:4] in ("dpbs", "dpdp"):
                assert (
                    opts.heatmap_threshold is None or opts.knn is None
                ), "Cannot have both"
                ...
                try:
                    sequences, costs, batch_size = evaluate_dp(
                        ...
                        heatmaps=heatmaps,
                        beam_size=beam_size,
                        collapse=opts.decode_strategy[:4] == "dpdp",
                ...
                costs = compute_batch_costs(
                    problem, batch, sequences, device=device, check_costs=costs
                )
        ...
        for seq, cost in zip(sequences, costs):
            if problem.NAME in ("tsp", "tsptw"):
                if (
                    seq is not None
                ):  # tsptw can be infeasible or TSP failed with sparse graph
...
if __name__ == "__main__":
    ...
    assert (
        opts.o is None or (len(opts.datasets) == 1 and len(opts.beam_size) <= 1)
