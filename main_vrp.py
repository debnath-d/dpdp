...
def train_one_epoch(net, optimizer, config, master_bar, dataset=None):
    ...
    for batch_num in progress_bar(range(batches_per_epoch), parent=master_bar):
        ...
        x_edges_values = Variable(
            torch.FloatTensor(batch.edges_values).type(dtypeFloat), requires_grad=False
        )
        x_nodes = Variable(
        ...
        y_edges = Variable(
            torch.LongTensor(batch.edges_target).type(dtypeLong), requires_grad=False
        )
...
def test(net, config, master_bar, mode="test"):
    ...
    if mode == "val":
        dataset = VRPReader(
            num_nodes, num_neighbors, batch_size, val_filepath, val_target_filepath
        )
    elif mode == "test":
        dataset = VRPReader(
            num_nodes, num_neighbors, batch_size, test_filepath, test_target_filepath
        )
    batches_per_epoch = dataset.max_iter
    ...
    with torch.no_grad():
        ...
        for batch_num in progress_bar(range(batches_per_epoch), parent=master_bar):
            ...
            y_edges = Variable(
                torch.LongTensor(batch.edges_target).type(dtypeLong),
                requires_grad=False,
            )
            # y_nodes = Variable(torch.LongTensor(batch.nodes_target).type(dtypeLong), requires_grad=False)
