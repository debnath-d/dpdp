...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--unsafe",
        action="store_true",
        help="Force merging results even with different settings",
    )
    ...
    for dataset, ds_opts in sorted(datasets, key=lambda k: k[1].offset):
        ...
        if save_opts is None:
            ...
            assert (
                ds_opts.offset == save_opts.offset + save_opts.val_size
            ), "Offset does not match previous!"
            save_opts.val_size += ds_opts.val_size

        for k in vars(ds_opts):
            if not (
                k in ("offset", "val_size", "o")
                or getattr(save_opts, k) == getattr(ds_opts, k)
            ):
        ...
        if ds_opts.val_size < ds_opts.batch_size:
            print(
                "Warning, inaccurate total time computation if val_size per split is smaller than batch_size"
            )
