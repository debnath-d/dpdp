...
assert (
    not os.path.isfile(heatmap_filename) or args.f
), "Use -f to overwrite existing results"
...
model_class = (
    ResidualGatedGCNModelVRP if args.problem == "vrp" else ResidualGatedGCNModel
)
...
if args.problem == "vrp":
    ...
    reader = DataReader(
        num_nodes, num_neighbors, batch_size, instance_filepath, do_prep=not do_prepwrap
    )

assert (
    len(reader.filedata) % batch_size == 0
), f"Number of instances {len(reader.filedata)} must be divisible by batch size {batch_size}"
...
for i, batch in enumerate(tqdm(dataset, total=reader.max_iter)):
    with torch.no_grad():
        if args.problem in ("tsp", "tsptw") and do_prepwrap:
            ...
            # Convert batch to torch Variables
            x_edges = Variable(
                torch.LongTensor(batch.edges).type(dtypeLong), requires_grad=False
            )
            x_edges_values = Variable(
                torch.FloatTensor(batch.edges_values).type(dtypeFloat),
                requires_grad=False,
            )
            x_nodes = Variable(
                torch.LongTensor(batch.nodes).type(dtypeLong), requires_grad=False
            )
            x_nodes_coord = Variable(
                torch.FloatTensor(batch.nodes_coord).type(dtypeFloat),
                requires_grad=False,
            ...
            with torch.no_grad():
                y_preds, loss = net.forward(
                    x_edges, x_edges_values, x_nodes, x_nodes_coord
                )
...
save_dataset(
    (
        heatmaps.numpy(),
        {"duration": duration, "device_count": device_count, "args": args},
    ),
    heatmap_filename,
)
