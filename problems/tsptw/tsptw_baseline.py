...
def calc_tsptw_cost(dist, timew, tour, makespan=False):
    ...
    assert (
        np.sort(tour) == np.arange(len(timew) - 1) + 1
...
def write_tsptwlib(filename, dist, timew, name="problem"):
    with open(filename, "w") as f:
        ...
        f.write(
            "\n".join(
                ["{}\t{}\t{}".format(i + 1, l, u) for i, (l, u) in enumerate(timew)]
            )
        )
        f.write("\n")
    ...
    try:
        ...
        if os.path.isfile(output_filename) and not disable_cache:
            ...
            with open(log_filename, "w") as f:
                ...
                seed = 1234
                check_call(
                    [executable, str(seed), str(runs), problem_filename],
                    stdout=f,
                    stderr=f,
                )
                duration = time.time() - start
...
def write_tsptw(filename, coord, timew, name="problem"):
    with open(filename, "w") as f:
        ...
        for i, ((x, y), (l, u)) in enumerate(zip(coord, timew)):
            f.write(
                "{:>5d} {:>10.6f} {:>10.6f} {:>10.2f} {:>10.2f} {:>10.2f} {:>10.2f}\n".format(
                    i + 1, x, y, 0, l, u, 0
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "method", help="Name of the method to evaluate, 'lkh' or 'gvns'"
    )
    parser.add_argument(
