...
class CVRP:
    ...
    def get_costs(dataset, pi, allow_infeas=False):
        ...
        vehicle_capacity = dataset.get(
            "capacity", dataset["demand"].new_full((batch_size,), CVRP.VEHICLE_CAPACITY)
        ).to(dataset["demand"].dtype)
        # Visiting depot resets capacity so we add demand = -capacity (we make sure it does not become negative)
        demand_with_depot = torch.cat(
            (-vehicle_capacity.view(batch_size, 1), dataset["demand"]), 1
        )
        ...
        for i in range(pi.size(1)):
            ...
            assert (
                allow_infeas or (used_cap <= vehicle_capacity + 1e-5).all()
        ...
        if filename is not None:
            ...
            if distribution == "nazari":
                ...
                self.data = [
                    {
                        ...
                        "demand": (
                            torch.FloatTensor(size).uniform_(0, 9).int() + 1
                        ).float()
                        / CAPACITIES[size],
                ...
                self.data = [
                    ...
                    for depot, loc, demand, capacity, depot_types, customer_types, grid_size in zip(
                        *generate_uchoa_instances(num_samples, size, distribution)
                    )
                ]
