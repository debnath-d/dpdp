...
class BatchNormNode(nn.Module):
    ...
    def forward(self, x):
        ...
        x_trans = x.transpose(
            1, 2
        ).contiguous()  # Reshape input: (batch_size, hidden_dim, num_nodes)
...
class BatchNormEdge(nn.Module):
    ...
    def forward(self, e):
        ...
        e_trans = e.transpose(
            1, 3
        ).contiguous()  # Reshape input: (batch_size, hidden_dim, num_nodes, num_nodes)
...
class NodeFeatures(nn.Module):
    ...
    def __init__(self, hidden_dim, aggregation="mean"):
        super().__init__()  # We must always sum, since mean means 'weighted mean' so sum weighted messages
...
class EdgeFeatures(nn.Module):
    ...
    def forward(self, x, e, edge_index=None):
        ...
        Vx = self.V(x)
        Wx = (
            Vx if self.W is None else self.W(x)
        )  # If self.W is none, graph is undirected
        if edge_index is not None:
            # Sparse version
            src, dst = edge_index
            Wx = Wx[dst]  # = to
