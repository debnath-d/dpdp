...
def wrap_sparse(model, type="dense"):
    if type[:3] == "knn":
        ...
        return KnnResidualGatedGCNModel(
            model, k=int(k), include_self_loops=knn_type == "knns"
        )
    elif type == "dense":
...
class SparseResidualGatedGCNModel(nn.Module):
    def __init__(self, model):
        super().__init__()
        self.model = model
        self.logit_noedge = nn.Parameter(torch.tensor([0.0]))
    ...
    def forward(
        ...
        if CHECK_DENSE:
            y_preds_check, loss_check = self.model(
                x_edges, x_edges_values, x_nodes, x_nodes_coord, y_edges, edge_cw
            )

        batch_size, num_nodes = x_nodes.size()
        # Edge_idx
        edge_batch, edge_i, edge_j = self.get_edges(
            x_edges, x_edges_values, x_nodes, x_nodes_coord, y_edges, edge_cw
        )
        node_batch, node_i = torch.ones(
            batch_size, num_nodes, device=x_nodes.device
        ).nonzero(as_tuple=True)

        edge_node_offset = edge_batch * num_nodes
        edge_index = torch.stack(
            (edge_node_offset + edge_i, edge_node_offset + edge_j), 0
        )
        # Now index all
        ...
        y_preds, loss_sparse = self.model(
            x_edges,
            x_edges_values,
            x_nodes,
            x_nodes_coord,
        ...
        y_preds_dense = y_preds.new_zeros(
            (batch_size, num_nodes, num_nodes, y_preds.size(-1))
        )
        ...
        if edge_cw is not None:
            if not torch.is_tensor(edge_cw):
                edge_cw = torch.Tensor(edge_cw).type(
                    self.model.dtypeFloat
                )  # Convert to tensors
    ...
    def get_edges(
        ...
        batch_size, num_nodes = x_nodes.size()
        return torch.ones(
            batch_size, num_nodes, num_nodes, device=x_nodes.device
        ).nonzero(as_tuple=True)
...
class KnnResidualGatedGCNModel(SparseResidualGatedGCNModel):
    ...
    def get_edges(
        ...
        batch = (
            torch.arange(batch_size, device=x_nodes_coord.device)[:, None]
            .repeat(1, num_nodes)
            .flatten()
        )
        x_nodes_coord_flat = x_nodes_coord.flatten(0, 1)
        edge_index = knn_graph(
            x_nodes_coord_flat, self.k, batch, loop=self.include_self_loops
        )
