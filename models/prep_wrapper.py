...
class PrepWrapResidualGatedGCNModel(nn.Module):
    ...
    def forward(self, x_nodes_coord, x_nodes_timew=None, y_tour=None, edge_cw=None):
        ...
        x_edges_values = torch.cdist(
            x_nodes_coord, x_nodes_coord, compute_mode="donot_use_mm_for_euclid_dist"
        )
        # 1 for edge, 2 for self-loop (fully connected)
        x_edges = (
            torch.eye(x_nodes.size(-1), dtype=torch.long, device=x_nodes.device) + 1
        )[None].expand_as(x_edges_values)
        y_edges = None
        ...
        y_preds, loss = self.model(
            x_edges,
            x_edges_values,
            x_nodes,
            x_nodes_coord,
            x_nodes_timew=x_nodes_timew,
            y_edges=y_edges,
            edge_cw=edge_cw,
        )
