...
def generate_tsptw_data(dataset_size, tsp_size, distribution, seed):
    from problems.tsptw.generate_cappart_instance import (
        generate_random_instance as generate_cappart_tsptw_instance,
    )
    ...
    fork_seed = (
        distribution != "capparti"
    )  # Cappart et al. dataset with incremental seeds (seed, seed + 1, ...)
    np.random.seed(opts.seed)
    ...
    seeds = (
        np.random.randint(0, np.iinfo(np.int32).max, dataset_size)
        if fork_seed
        else seed + np.arange(dataset_size)
    )
    ...
    return [
        generate_cappart_tsptw_instance(
            n_city=tsp_size,
            grid_size=100,
            max_tw_gap=10,
            ...
            seed=s,
            da_silva_style=distribution == "da_silva",
        )
        for s in tqdm(seeds)
    ]
...
def generate_uchoa_vrp_data(dataset_size, vrp_size, distribution, seed):
    ...
    depot, loc, demand, capacity, depot_types, customer_types, grid_size = (
        generate_uchoa_instances(dataset_size, vrp_size, distribution)
    )
    return list(
        zip(
            depot.tolist(),
...
if __name__ == "__main__":
    ...
    assert opts.filename is None or (
        opts.problem != "all" and len(opts.graph_sizes) == 1
    ...
    assert (
        opts.filename is None or len(problems[opts.problem]) == 1
    ), "Can only specify single distribution when generating single dataset"
    ...
    for problem, distributions in problems.items():
        for distribution in distributions or [None]:
            for graph_size in opts.graph_sizes:
                ...
                np.random.seed(
                    opts.seed
                )  # Just in case if the method does not seed itself
                if problem == "tsp":
                    dataset = generate_tsp_data(
                        opts.dataset_size, graph_size, seed=opts.seed
                    )
                elif problem == "vrp":
