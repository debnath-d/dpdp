...
class Profiler:
    ...
    def print_profile_summary(self, step, detailed=False):
        ...
        print(
            "-------------- Step {} total duration: {:.3f} ms -------------------".format(
                step, total_duration * 1000
            )
        )
        event_durations = np.diff(event_times)
        event_names = event_names[1:]
        total_generate = sum(
            d for e, d in zip(event_names, event_durations) if "expansion" in e
        )
        total_reduce = sum(
            d for e, d in zip(event_names, event_durations) if "reduced" in e
        )
        ...
        print(
            "Total rest time                {:.3f} ms".format(
                (total_duration - total_generate - total_reduce) * 1000
            )
        )
        ...
        if detailed:
            for i in np.argsort(-event_durations):  # Sort descending by duration
                print(
                    ("{:" + str(maxlen) + "s} {:.3f} ms").format(
                        event_names[i], event_durations[i] * 1000
                    )
                )
