...
def get_durations(durations, batch_size, num_processes, device_count):
    ...
    total_duration_parallel = timedelta(
        seconds=int(total_duration_processes / num_processes + 0.5)
    )
    total_duration_single_device = timedelta(
        seconds=int(
            total_duration_processes / (num_processes / max(device_count, 1)) + 0.5
        )
    )
    return (
        avg_serial_duration,
...
def ensure_backward_compatibility(opts):
    ...
    if "used_num_processed" in opts.system_info:
        opts.system_info["used_num_processes"] = opts.system_info.pop(
            "used_num_processed"
        )
...
def compute_batch_costs(
    problem, batch, sequences, device=None, check_costs=None, warn_only=False
):
    idx_success = torch.tensor(
        [i for i, seq in enumerate(sequences) if seq is not None], device=device
    )
    ...
    if check_costs is not None:
        ...
        isclose = torch.isclose(
            costs_success, costs_success.new_tensor(check_costs_success), atol=1e-10
        )
        if not isclose.all():
            idx_notclose = (~isclose).nonzero().flatten()
            print(
                "Warning check cost {} not exactly equal to {} for idx {}".format(
                    costs_success[idx_notclose],
                    check_costs_success[idx_notclose],
                    idx_notclose,
                )
            )
        assert warn_only or torch.allclose(
            costs_success, costs_success.new_tensor(check_costs_success), atol=1e-4
        ), "Check cost {} not equal to {}".format(costs_success, check_costs_success)
