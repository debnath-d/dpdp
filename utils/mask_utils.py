...
def packmask(mask):
    ...
    return torch.from_numpy(
        np.pad(
            bits_np,
            ((0, 0), (0, ((bits_np.shape[-1] + 7) // 8) * 8 - bits_np.shape[-1])),
        ).view(np.int64)
    ).to(mask.device)
...
def unpack_mask(mask, view_as_binary=False, num_nodes=None, do_check=False):
    ...
    if mask.device != torch.device("cpu"):
        ...
        assert (
            mask.stride(-1) == 1
        ), "mask.stride(-1) should be 1 for unpack_mask to work!!"
        assert mask.dim() == 2
        ...
        if view_as_binary:
            ...
            result = (
                unpacked.view(mask.size(0), -1, 8)
                .flip(-1)
                .view(mask.size(0), -1)[:, :num_nodes]
            )
        if do_check:
            checkmask = unpack_mask(
                mask.cpu(), view_as_binary=view_as_binary, num_nodes=num_nodes
            ).to(mask.device)
    ...
    mask_np = (
        ...
        else np.column_stack([to_np(col) for col in mask])
    )
    # Bitorder little so that when we print the mask, the leftmost (first digit) corresponds to 1
    unpacked_mask = np.unpackbits(
        np.ascontiguousarray(mask_np).view(np.uint8), axis=-1, bitorder="little"
    )[:, :num_nodes]
    if view_as_binary:
        unpacked_mask = unpacked_mask[
            :, ::-1
        ]  # So when digit 1 is set it will be the rightmost when printing
    return (
        torch.from_numpy(unpacked_mask).to(mask.device)
