...
from utils.scatter_utils import coo_to_csr, counts_to_csr, csr_to_coo, csr_to_counts
...
class MergedGraph:
    ...
    def __init__(self, graphs):
        ...
        def get_stacked_props(name):
            ...
            if torch.is_tensor(props[0]):
                ...
                if dim == 2:
                    ...
                    props = [
                        F.pad(
                            mat, (max_cols - mat.size(1), 0, max_rows - mat.size(0), 0)
                        )
                        for mat in props
                    ]
                elif dim == 1:
        ...
        self.node_csr = counts_to_csr(
            self.batch_num_nodes
        )  # F.pad(torch.cumsum(self.num_nodes, 0), (1, 0))
        self.node_offset = self.node_csr[:-1]
        self.node_coo = csr_to_coo(self.node_csr)
        self.nodes = (
            torch.arange(self.batch_num_nodes.sum(), dtype=torch.long, device=device)
            - self.node_offset[self.node_coo]
        )
    ...
    def quantize_score(self, score):
        return (
            ...
            else quantize(score, self.quantize_score_ub, self.score.dtype)
        )
    ...
    def dequantize_score(self, score):
        return (
            score
            if self.quantize_score_ub is None
            else dequantize(score.to(self.score.dtype), self.quantize_score_ub)
        )
    ...
    def quantize_cost(self, cost):
        return (
            ...
            else quantize(cost, self.quantize_cost_ub, self.cost.dtype)
        )
    ...
    def dequantize_cost(self, cost):
        return (
            cost
            if self.quantize_cost_ub is None
            else dequantize(cost.to(self.cost.dtype), self.quantize_cost_ub)
        )
...
class BatchGraph:
    def __init__(
        ...
        if quantize_cost is not None:
            ...
            assert (
                timew is None or timew.dtype == cost.dtype
            ), "Cost and time windows must have same dtype"
            cost = quantize(cost, self.quantize_cost_ub, dtype=quantize_cost_dtype)
            if timew is not None:
                timew = quantize(
                    timew, self.quantize_cost_ub, dtype=quantize_cost_dtype
                )
        ...
        self.adj_in_packed = packmask(adj_t.reshape(-1, adj_t.size(-1))).view(
            *adj_t.size()[:-1], -1
        )
        self.adj_out_packed = packmask(adj.reshape(-1, adj.size(-1))).view(
            *adj.size()[:-1], -1
        )
        ...
        self.batch_num_nodes = torch.full(
            (self.batch_size,), self.num_nodes, dtype=torch.long, device=device
        )
        ...
        self.node_coo = csr_to_coo(self.node_csr)
        self.nodes = (
            torch.arange(len(self.node_coo), dtype=torch.long, device=device)
            - self.node_offset[self.node_coo]
        )
        ...
        self.total_edge_weight_out = (
            None if edge_weight is None else edge_weight.sum(-1)
        )
    ...
    def get_graph(
        ...
        score_function="cost",
        heatmap=None,
        heatmap_threshold=1e-5,
        knn=None,
        quantize_cost_ub=None,
        quantize_cost_dtype=None,
        normalize="incoming",
        demand=None,
        vehicle_capacity=None,
        depot_penalty=math.log(0.1),
        timew=None,
        start_node=0,
        node_score_weight=1.0,
        node_score_dist_to_start_weight=0.1,
    ):
        ...
        if score_function == "costheatmap":
            ...
            heatmap = (
                ...
                - dist
                / torch.minimum(
                    dist.max(-1, keepdim=True).values, dist.max(-2, keepdim=True).values
                )
            ).log()
        ...
        if heatmap_threshold is not None:
            ...
            # fully connected
            adj_mask = (
                torch.eye(dist.size(-2), device=dist.device, dtype=bool)
                .logical_not_()[None]
                .expand_as(dist)
            )
        ...
        if score_function == "heatmap":
            ...
            if add_potentials:
                norm_dim = (
                    -1 if normalize == "outgoing" else -2
                )  # -1 = keep potentials based on outgoing edges, otherwise on incoming edges
                ...
                if node_score_dist_to_start_weight != 0:
                    assert (
                        demand is None or start_node == 0
                    ), "Start node must be 0 (depot) for VRP"
                    ...
                    normalized_dist_to_start = (
                        dists_to_start / dists_to_start.max(-1, keepdim=True).values
                    )
                    node_score = node_score * (
                        node_score_weight
                        - node_score_dist_to_start_weight
        ...
        if quantize_cost_dtype is not None:
            ...
            quantize_ub = quantize_cost_ub or (dist * adj_mask).max(-1).values.sum(
                -1
            ) * (1 if demand is None else 2)
        ...
        if score_function == "heatmap":
            ...
            graph.score_via_depot = graph.quantize_score(
                -(logp_via_depot if use_logp else logp_via_depot.exp())
            )
            graph.score_from_depot = graph.quantize_score(
                -(logp_from_depot if use_logp else logp_from_depot.exp())
            )
    ...
    def dequantize_score(self, score):
        ...
        if not torch.is_tensor(score):
            return [
                dequantize(s.to(self.score.dtype), ub).item() if s is not None else None
                for s, ub in zip(score, self.quantize_score_ub)
            ]
    ...
    def dequantize_cost(self, cost):
        ...
        if not torch.is_tensor(cost):
            return [
                dequantize(c.to(self.cost.dtype), ub).item() if c is not None else None
                for c, ub in zip(cost, self.quantize_cost_ub)
            ]
...
class Graph:
    def __init__(
        ...
        self.adj = adj
        self.adj_in_list = [
            adj_col.nonzero(as_tuple=False).flatten() for adj_col in adj.t()
        ]
        self.adj_out_list = [
            adj_row.nonzero(as_tuple=False).flatten() for adj_row in adj
        ]
        ...
        assert (cost[tuple(edges_in.indices())] == self.edge_in_cost).all()
        self.edge_out_score = (
            self.edge_out_cost if score is None else score[tuple(edges_out.indices())]
        )
        self.edge_in_score = (
            self.edge_in_cost if score is None else score[tuple(edges_in.indices())]
        )
        self.edge_out_idx_csr_from = coo_to_csr(edges_out.indices()[0])
        self.edge_out_idx_to = edges_out.indices()[1]
        self.edge_in_idx_from = edges_in.indices()[1]
    ...
    def get_graph(
        ...
        if score_function == "costheatmap":
            ...
            heatmap = (
                ...
                - dist
                / torch.minimum(
                    dist.max(0, keepdim=True).values, dist.max(1, keepdim=True).values
                )
            ).log()
            score_function = "heatmap"  # Treat as if real heatmap
            assert heatmap_threshold is None, "Not compatible with threshold"
        ...
        if heatmap_threshold is not None:
            ...
            # fully connected
            adj_mask = torch.eye(
                coord.size(0), device=dist.device, dtype=bool
            ).logical_not_()
