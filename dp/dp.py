import torch
from torch_scatter import segment_max_coo, segment_min_coo
...
from .vrp import get_vrp_cost, vrp_actions_to_solutions, vrp_solution_to_actions
...
def run_dp(
    ...
    trace_device="cpu",
    enforce_return_step_feasible="hard",
    verbose=False,
    profile_every=-1,
    ...
    potential_info = (
        get_initial_potential_info(graph, start_node) if add_potentials else None
    )
    ...
    beam = Beam(
        ...
        cost_dtype=graph.cost.dtype,
        device=beam_device,
        sort_by=sort_beam_by,
        columns_per_group=1 if candidate_queue.capacity >= int(1e8) else None,
    ...
    trace = []
    for step in range(
        graph.num_nodes if graph.is_vrp else graph.num_nodes - 1
    ):  # For TSP, we have a start node so only n - 1 steps
        if verbose:
        ...
        beam = dp_step(
            ...
            profile=profile_every > 0 and step % profile_every == 0,
            verbose=verbose,
        ...
        if return_solution:
            trace.append(
                (beam.parent.to(trace_device), beam.last_action.to(trace_device))
            )
...
def finalize_solutions(
    ...
    is_return_feasible = (
        graph.adj_in_depot if graph.is_vrp else graph.adj[:, :, start_node]
    )[beam.batch_ids, current]
    cost_to_return = (
        graph.cost_to_depot if graph.is_vrp else graph.cost[:, :, start_node]
    )[beam.batch_ids, current]
    final_cost = beam.cost + cost_to_return
    assert (beam.batch_ids[1:] >= beam.batch_ids[:-1]).all()
    min_final_cost, best_ind = segment_min_coo(
        final_cost, beam.batch_ids, dim_size=graph.batch_size
    )
    has_solution = best_ind < beam.size
    if enforce_return_step_feasible in ("soft", "hard"):
        any_feasible = segment_max_coo(
            is_return_feasible.int(), beam.batch_ids, dim_size=graph.batch_size
        )[0].bool()
        cost_infeas = (
            torch.finfo if torch.is_floating_point(final_cost) else torch.iinfo
        )(final_cost.dtype).max
        final_cost_feasible = torch.where(
            is_return_feasible, final_cost, final_cost.new_tensor(cost_infeas)
        )
        min_final_cost_feasible, best_ind_feasible = segment_min_coo(
            final_cost_feasible, beam.batch_ids, dim_size=graph.batch_size
        )
        if enforce_return_step_feasible == "hard":
            ...
            min_final_cost = torch.where(
                any_feasible, min_final_cost_feasible, min_final_cost
            )
    ...
    if return_solution:
        ...
        trace.append(
            (best_ind, torch.full_like(best_ind, -1 if graph.is_vrp else start_node))
        )
        ...
        if graph.is_vrp:
            ...
            check_cost = get_vrp_cost(
                solution.to(graph.demand.device),
                graph.demand[batch_ids],
                graph.vehicle_capacity[batch_ids],
                graph.cost_incl_depot[batch_ids],
            )
        else:
            solution = solutions.roll(1, dims=-1)  # Start with start node
            assert (solution[:, 0] == start_node).all()  # Just checking!
            check_cost = graph.cost[
                batch_ids[:, None], solution, solution.roll(-1, dims=-1)
            ].sum(-1)
        if verbose:
            print(solution, min_final_cost)
        assert torch.allclose(
            check_cost.to(min_final_cost.dtype), min_final_cost
        ), "Check cost: {} not equal to {}".format(check_cost, min_final_cost)
...
def dp_step(
    ...
    actions, parents, scores = get_expansions(
        beam,
        bound_first,
        candidate_queue,
        collapse,
