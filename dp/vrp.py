...
def vrp_actions_to_solutions(actions, num_nodes=None):
    ...
    while (cur_idx < n_steps).any():
        ...
        a_depot = (
            ((a >= num_nodes) & (a_converted != 0)) | (cur_idx >= n_steps)
        ).long()  # a / n
        a_converted = (a_node + 1) * (
            1 - a_depot
        )  # 0 if depot or finished, else a_node + 1
...
def get_vrp_cost(solution, demand, vehicle_capacity, cost_incl_depot):
    ...
    demands_per_route = segment_sum_coo(
        F.pad(demand, (1, 0)).gather(-1, solution), route_idx
    )
