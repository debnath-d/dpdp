...
def compute_vrp_expansion_solution_potentials(
    graph,
    mask_unvisited,
    beam,
    add_from_current_node=False,
    add_to_return_node=True,
    DO_CHECK=False,
):
    # Since the expansion always has a node visited, this function also works for empty solution
    (
        node_remaining_edge_weight_in,
        node_remaining_edge_weight_out,
        depot_remaining_edge_weight_in,
        depot_remaining_edge_weight_out,
    ) = beam.potential_info

    if DO_CHECK:
        beam_edge_weight = graph.edge_weight[beam.batch_ids]
        node_remaining_edge_weight_in_check = (
            mask_unvisited[:, :, None] * beam_edge_weight
        ).sum(-2)
        ...
        node_remaining_edge_weight_out_check = (
            mask_unvisited[:, None, :] * beam_edge_weight
        ).sum(-1)
        assert torch.allclose(
            node_remaining_edge_weight_out,
    ...
    unvisited_potentials_expansions = (
        unvisited_potentials[:, None] - node_remaining_edge_weight_in
    )
    ...
    if not add_from_current_node:
        ...
        unvisited_potentials_expansions = (
            unvisited_potentials_expansions - node_remaining_edge_weight_out
        )
        if add_to_return_node:
            # We have added earlier from the new current node to the depot node so we should subtract this again
            unvisited_potentials_expansions = (
                unvisited_potentials_expansions
                - graph.to_depot_edge_weight[beam.batch_ids, :]
            )

    return unvisited_potentials_expansions


def compute_tsp_expansion_solution_potentials(
    graph,
    mask_unvisited,
    beam,
    add_from_current_node=False,
    ...
    """
    assert (
        beam.start_node is not None
    ), "TODO: does this implementation work without start node?"
    ...
    if add_to_return_node:
        ...
        unvisited_potentials = (
            unvisited_potentials + node_remaining_edge_weight_in[:, beam.start_node]
        )
    ...
    # Always subtract the nodes to the new current node since that will be visited after expanding
    if (
        beam.start_node is not None or not add_to_return_node
    ):  # For TSP since we should return to first node still
        unvisited_potentials_expansions = (
            unvisited_potentials[:, None] - node_remaining_edge_weight_in
        )
    else:
        ...
        unvisited_potentials_expansions = unvisited_potentials[:, None].expand_as(
            node_remaining_edge_weight_in
        )
    ...
    if not add_from_current_node:
        ...
        if add_to_return_node and beam.start_node is not None:
            ...
            unvisited_potentials_expansions = (
                unvisited_potentials_expansions
                - graph.edge_weight[beam.batch_ids, :, beam.start_node]
            )
...
def update_potential_info(graph, potential_info, parent, visited_node, parent_batch_id):
    ...
    (
        prev_node_remaining_edge_weight_in,
        prev_node_remaining_edge_weight_out,
        *vrp_potential_info,
    ) = potential_info

    # Update remaining weights in by subtracting the edge_weights from the current node to each node
    node_remaining_edge_weight_in = (
        prev_node_remaining_edge_weight_in[parent]
        - graph.edge_weight[parent_batch_id, visited_node, :]
    )
    # Update remaining weights out by subtracting the edge_weights to the current node from each node
    node_remaining_edge_weight_out = (
        prev_node_remaining_edge_weight_out[parent]
        - graph.edge_weight[parent_batch_id, :, visited_node]
    )
    ...
    # VRP
    prev_depot_remaining_edge_weight_in, prev_depot_remaining_edge_weight_out = (
        vrp_potential_info
    )
    depot_remaining_edge_weight_in = (
        prev_depot_remaining_edge_weight_in[parent]
        - graph.to_depot_edge_weight[parent_batch_id, visited_node]
    )
    depot_remaining_edge_weight_out = (
        prev_depot_remaining_edge_weight_out[parent]
        - graph.from_depot_edge_weight[parent_batch_id, visited_node]
    )
    return (
        node_remaining_edge_weight_in,
        node_remaining_edge_weight_out,
