...
from utils.sorting_utils import unique1d, unique_inverse
...
class Beam:
    def __init__(
        ...
        assert (
            start_node == 0
        ), "Only support start node 0 for now, reorder for different start node"
        self.batch_ids = torch.arange(
            batch_size, device=device
        )  # 0 if batch_size == 1 else
        ...
        self.col_idx_per_group = torch.arange(
            self.mask_num_cols, dtype=torch.long, device=device
        ).split(self.columns_per_group)
        ...
        self.mask = [
            torch.zeros(len(col_group), batch_size, dtype=mask_dtype, device=device)
            for col_group in self.col_idx_per_group
        ]
        if vehicle_capacity is None:  # Not for VRP!
            self.mask[0][0].fill_(
                1
            )  # Start node at 1 which mask in the bitorder little is represented as 0000001 = 1 (first column of first colgroup)
        ...
        self.score = (
            torch.zeros(batch_size, dtype=score_dtype, device=device)
            if score_dtype is not None
            else None
        )
        self.vehicle_capacity = vehicle_capacity
        self.remaining_capacity = (
            torch.as_tensor(vehicle_capacity, device=device).flatten()
            if vehicle_capacity is not None
            else None
        )  # Dtype inferred from capacity
    ...
    def update(
        ...
        mask_cols = [
            col for col_group in mask for col in col_group
        ]  # Flatten all columns in the mask
        if (
            torch.is_tensor(batch_ids)
            and len(batch_ids) > 0
            and (batch_ids != batch_ids[0]).any()
        ):
            # Prepend batch id to mask_cols so we get unique masks per graph
            mask_cols.insert(0, batch_ids)
        group_idx, mask_idx_per_group = unique_inverse(
            mask_cols, return_index=True, device=compute_unique_device
        )
        ...
        current_csr = (
            None  # We don't have a current csr representation unless we sort by current
        )
        current_counts = None
        if self.sort_by is not None:
            if self.sort_by == "current":
                assert (
                    self.batch_size == 1
                ), "Sorting by current not compatible with batch ids"
            ...
            current = (
                current
                if self.sort_by == "current"
                else torch.gather(current, 0, argsort)
            )
            group_idx = (
                group_idx
                if self.sort_by == "group_idx"
                else torch.gather(group_idx, 0, argsort)
            )
            ...
            if potential_info is not None:
                potential_info = tuple(
                    p_info.index_select(0, argsort) for p_info in potential_info
                )
        ...
        self.last_action = (
            last_action if last_action is not None else current
        )  # For TSP, last action is current
        self.potential_info = potential_info
        self.parent = parent
    ...
    def summary(self):
        ...
        _, counts_mask = unique1d(
            self.group_idx if self.group_idx is not None else self.mask_idx,
            return_counts=True,
        )
        ...
        unique_current, counts_current = (
            unique1d(self.current, return_counts=True)
        ...
        return (
            ...
            + "Singles {} ({:.1f} %), multiples {} ({:.1f} %) in {} groups of avg/max size {}/{}\n".format(
                ...
                num_multiple,
                "{:.2f}".format((self.size - num_unique) / num_multiple)
                if num_multiple > 0
                else "-",
            ...
            + (
                "Unique current nodes: {} with between {} - {} states".format(
                    unique_current.size(0), counts_current.min(), counts_current.max()
                )
                if unique_current is not None
                else ""
            )
        )
    ...
    def destroy(self):
        ...
        assert (
            self.start_node == 0
        )  # Note: we don't clear the start node as it remains unchanged (a scalar 0)
        self.parent = None
...
def update_beam(actions, beam, compute_unique_device, graph, parents, profiler, scores):
    ...
    all_parent_batch_id = (
        beam.batch_ids.gather(0, all_parent_beamrows_l) if graph.is_batch else 0
    )
    if graph.is_vrp or beam.score is not None:
        ...
        if graph.is_vrp:
            all_remaining_capacity = (
                torch.where(
                    actions
                    >= graph.num_nodes,  # num_nodes excludes depot, so 0 to num_nodes - 1 are direct
            ...
            if graph.is_vrp:
                ...
                if beam.current is None:
                    ...
                    if beam.potential_info is not None:
                        scores = graph.score_from_depot[
                            all_parent_batch_id, all_current_l
                        ]
                else:
                    prev = beam.current.gather(0, all_parent_beamrows_l).long()
                    all_cost = beam.cost.gather(0, all_parent_beamrows_l).add_(
                        torch.where(
                            actions >= graph.num_nodes,
                            graph.cost_to_depot[all_parent_batch_id, prev].add_(
                                graph.cost_from_depot[
                                    all_parent_batch_id, all_current_l
                                ]
                            ),
                    ...
                    if beam.potential_info is not None:
                        scores = beam.score.gather(0, all_parent_beamrows_l).add_(
                            torch.where(
                                actions >= graph.num_nodes,
                ...
                if graph.has_tw:
                    all_time = (
                        torch.maximum(  # Ensure arrival is not before time window start
                            beam.time.gather(0, all_parent_beamrows_l) + dcost,
                            graph.timew[all_parent_batch_id, all_current_l, 0],
                        )
                    )
                    assert (
                        all_time <= graph.timew[all_parent_batch_id, all_current_l, 1]
                    ).all()
                if beam.potential_info is not None:
                    scores = (
                        beam.score.gather(0, all_parent_beamrows_l)
                        + graph.score[all_parent_batch_id, prev, all_current_l]
                    )
    ...
    if potential_info is not None:
        ...
        new_potential_info = update_potential_info(
            graph, potential_info, parents, all_current_node, all_parent_batch_id
        )
        potential_info = None
    ...
    beam.update(
        ...
        score=scores,
        time=all_time,
        compute_unique_device=compute_unique_device,
        remaining_capacity=all_remaining_capacity,
        last_action=actions if graph.is_vrp else None,
        potential_info=new_potential_info,
...
def update_mask(all_actions, all_parent_beamrows, mask, mask_idx, col_idx_per_group):
    ...
    parent_mask_idx = (
        all_parent_beamrows
        if mask_idx is None
        else torch.gather(mask_idx, 0, all_parent_beamrows.long())
    ).long()  # group_idx[all_parent_beamrows]  # Is already an index
    ...
    a_col = (
        torch.floor_divide(a, mask_bits) if len(mask) > 1 or len(mask[0]) > 1 else None
    )
    ...
    for i, (mask_col, col_idx) in enumerate(zip(mask, col_idx_per_group)):
        ...
        msk = (
            col_idx[:, None] == a_col[None, :]
            if a_col is not None
            else a_bit.new_tensor(1)
        )
    ...
    if do_checks:
        ...
        densemaskcheck = unpack_mask(
            mask[
                mask_idx[all_parent_beamrows]
                if mask_idx is not None
                else all_parent_beamrows
            ]
        )
