import torch
from torch_scatter import scatter_min, segment_min_coo
from utils.mask_utils import unpack_mask, unpackbits, view_as_uint8
from utils.scatter_utils import segment_cummax_coo, segment_cummin_coo, segment_sort_coo
from utils.sorting_utils import unique_consecutive, unique_consecutive_inverse

from dp.potentials import (
    compute_tsp_expansion_solution_potentials,
    compute_vrp_expansion_solution_potentials,
)
...
def get_expansions(
    ...
    unvisited_t = (
        beam.get_mask_colgroup(0, device=graph.cost.device, copy=True)
        .bitwise_not_()
        .t()
    )
    ...
    if beam.potential_info is not None:
        func = (
            compute_vrp_expansion_solution_potentials
            if graph.is_vrp
            else compute_tsp_expansion_solution_potentials
        )
        expansion_potentials = func(
            graph,
            unpack_mask(unvisited_t.contiguous(), num_nodes=graph.num_nodes),
            beam,
        )
    ...
    if graph.is_vrp:
        add_vrp_expansion_candidates_via_depot(
            beam,
            graph,
            candidate_queue,
            depot_info,
            ...
            profiler,
            unvisited_t,
            expansion_potentials=expansion_potentials,
            collapse=collapse,
        )
    ...
    if beam.current is not None:
        add_expansion_candidates_direct(
            ...
            profiler,
            unvisited_t,
            bound=candidate_queue.bound,
            expansion_potentials=expansion_potentials,
            collapse=collapse,
            use_weak_version=use_weak_version,
        )
    ...
    profiler.log(
        f"get_final_topk (total {total_candidates} candidates, reduced {reduced})"
    )
    if verbose:
...
def get_depot_info(beam, graph):
    ...
    beam_cost_at_depot = (
        ...
        else beam.cost + graph.cost_to_depot[beam.batch_ids, beam.current.long()]
    )
    if beam.sort_by == "group_idx":
        group_min_cost_at_depot, group_idx_min_cost_at_depot = segment_min_coo(
            beam_cost_at_depot, beam.group_idx
        )
    else:
        group_min_cost_at_depot, group_idx_min_cost_at_depot = scatter_min(
            beam_cost_at_depot, beam.group_idx
        )
...
def add_vrp_expansion_candidates_via_depot(
    beam,
    graph,
    candidate_queue,
    depot_info,
    ...
    if collapse:
        ...
        is_feasible_via_depot = unvisited_t.index_select(
            0, group_idx_min_cost_at_depot
        ).bitwise_and_(
            graph.adj_out_depot_packed[beam.batch_ids[group_idx_min_cost_at_depot]]
        )
        col_idx, row_idx = (
            unpackbits(view_as_uint8(is_feasible_via_depot)[:, :max_num_uint8s])
            .view(len(is_feasible_via_depot), -1)
            .t()
        ...
        is_feasible_via_depot = torch.bitwise_and(
            unvisited_t,
            graph.adj_out_depot_packed[beam.batch_ids],
            out=unvisited_t.new_empty(*unvisited_t.size()),
        )
        col_idx, row_idx = (
            unpackbits(view_as_uint8(is_feasible_via_depot)[:, :max_num_uint8s])
            .view(len(is_feasible_via_depot), -1)
        ...
        parent_idx_expansion = (
            row_idx  # Since we didn't filter unvisited_t in the first place
        )
    ...
    parent_batch_id = (
        beam.batch_ids[parent_idx_expansion] if graph.batch_size > 1 else 0
    )
    if graph.score is None:
        expansion_via_depot_cost = (
            group_min_cost_at_depot.gather(0, row_idx)
            if collapse
            else beam.cost.gather(0, parent_idx_expansion).add_(
                beam.cost_to_depot[
                    parent_batch_id, beam.current.long().gather(0, parent_idx_expansion)
                ]
            )
        ...
        if beam.current is None:
            ...
            expansion_via_depot_score = graph.score_from_depot[
                parent_batch_id, expand_node_idx
            ]
        else:
            # Note: probably order of gathering is important for efficiency
            expansion_via_depot_score = beam.score.gather(
                0,
                beam_idx_min_cost_at_depot.gather(0, parent_idx_expansion)
                if collapse
                else parent_idx_expansion,
            ).add_(
                graph.score_via_depot[
                    parent_batch_id,
                    beam.current.gather(0, parent_idx_expansion).long(),
                    expand_node_idx,
                ]
            )
        ...
        if expansion_potentials is not None:  # Add potentials to score
            expansion_via_depot_score.add_(
                expansion_potentials[parent_idx_expansion, expand_node_idx]
            )
    row_idx = None  # Make sure we don't use it anymore after filtering

    if candidate_queue.bound is not None:
        expansion_bound = (
            candidate_queue.bound
            if graph.batch_size == 1
            else candidate_queue.bound.gather(
                0, beam.batch_ids.gather(0, parent_idx_expansion)
            )
        )
    ...
    if len(parent_idx_expansion) > 0:
        expansion_via_depot_action = (
            expand_node_idx + graph.num_nodes
        )  # Actions n, n+1, ..., 2n-1 are via depot
        expansion_batch_ids = (
            beam.batch_ids.gather(0, parent_idx_expansion) if graph.is_batch else 0
        )
        candidate_queue.enqueue(
            expansion_via_depot_score,
            (parent_idx_expansion.int(), expansion_via_depot_action),
            batch_ids=expansion_batch_ids,
            already_bounded=True,
        )
        reduced = candidate_queue.get_num_items_reduced() or "NO"
        profiler.log(
            f"via_depot enqueue {len(parent_idx_expansion)} candidates, reduced {reduced}"
        )


def add_expansion_candidates_direct(
    beam,
    graph,
    candidate_queue,
    depot_info,
    ...
    parent, action = get_unvisited_expansion_candidates(
        beam, current, graph, max_num_uint8s, unvisited_t
    )
    ...
    if len(parent) == 0:
        return None
    profiler.log(
        f'filter {len(parent)} feasible_expansions with bound {bound if not graph.is_batch else "(batch)"}'
    )

    if collapse:
        collapse_fn = (
            collapse_vrp_expansions
            if graph.is_vrp
            else (
                collapse_tsptw_expansions if graph.has_tw else collapse_tsp_expansions
            )
        )
        parent, action, cost, batch_id = collapse_fn(
            batch_id, beam, cost, action, graph, parent
        )
        profiler.log(f"collapse {len(parent)} feasible_expansions")

    score = (
        compute_expansion_scores(
            beam, graph, current, parent, action, batch_id, expansion_potentials
        )
    ...
    candidate_queue.enqueue(
        score, (parent.int(), action.short()), batch_ids=batch_id, already_bounded=True
    )
    profiler.log(
        f'enqueue {len(parent)} candidates, reduced {candidate_queue.get_num_items_reduced() or "NO"}'
    )

    return parent, action, score, batch_id


def get_unvisited_expansion_candidates(
    beam, current, graph, max_num_uint8s, unvisited_t
):
    """
    ...
    :return:
    """
    is_feasible_all = graph.adj_out_packed[beam.batch_ids, current].bitwise_and_(
        unvisited_t
    )
    is_feasible_all_uint8 = view_as_uint8(is_feasible_all)[:, :max_num_uint8s]
    is_feasible_unpacked = (
        unpackbits(is_feasible_all_uint8.flatten())
        .view(*is_feasible_all_uint8.size()[:-1], is_feasible_all_uint8.size(-1) * 8)
        .t()
    )
...
def filter_feasible_expansions(
    beam,
    graph,
    current,
    parent,
    ...
    * remove expansions which makes some time windows no longer feasible
    ...
    if graph.is_vrp:
        msk = (
            beam.remaining_capacity.gather(0, parent)
            >= graph.demand[batch_id_feasible, action]
        )
        if collapse:
            (
                group_min_cost_at_depot,
            ...
            msk.logical_and_(
                cost_expansions
                < beam_min_cost_at_depot.gather(0, parent)
                + graph.cost_from_depot[batch_id_feasible, action]
            )
    if graph.has_tw:
        ...
        if not use_weak_version:
            unvisited_uint8 = view_as_uint8(unvisited_t.contiguous())[
                :, :max_num_uint8s
            ]
            unvisited_unpacked = (
                unpackbits(unvisited_uint8.flatten())
                .view(*unvisited_uint8.size()[:-1], unvisited_uint8.size(-1), 8)
                .flip(-1)
                .flatten(start_dim=-2)[:, : graph.num_nodes]
            )
            ...
            msk_tw.bitwise_and_(
                torch.maximum(arr, lb)
                + graph.cost[
                    batch_id_feasible, action, idx_first_due.gather(0, parent).long()
                ]
    ...
    if bound is not None:
        ...
        expansion_bound = (
            bound if graph.batch_size == 1 else bound.gather(0, batch_id_feasible)
        )
...
def collapse_vrp_expansions(
    batch_id_feasible,
    beam,
    cost_expansions,
    expand_node_idx,
    graph,
    idx_feasible_expansion,
):
    ...
    idx_scatter_expansions, group_sizes = unique_consecutive(
        idx_scatter_expansions,
        return_vals=False,
        return_inverse=True,
        return_counts=True,
    )
    ...
    sorted_cost_expansions, idx_segment_sort = segment_sort_coo(
        cost_expansions, idx_scatter_expansions, max_group_size=group_sizes.max()
    )
    ...
    remaining_capacity = (
        beam.remaining_capacity.gather(0, idx_feasible_expansion)
        - graph.demand[batch_id_feasible, expand_node_idx]
    )
    ...
    cummax_remaining_capacity = segment_cummax_coo(
        remaining_capacity, idx_scatter_expansions
    )
    msk_feas[1:].logical_and_(
        (idx_scatter_expansions[1:] != idx_scatter_expansions[:-1]).logical_or_(
...
def collapse_tsptw_expansions(
    ...
    expand_node_idx = expand_node_idx.gather(0, idx_segment_sort)
    arr = (
        beam.time.gather(0, idx_feasible_expansion)
        + graph.cost[
            batch_id_feasible,
            beam.current.gather(0, idx_feasible_expansion).long(),
            expand_node_idx,
        ]
    )
...
def compute_expansion_scores(
    ...
    score_expansions = (
        torch.gather(beam.score, 0, parent)
        + graph.score[batch_id, current.gather(0, parent).long(), action_l]
    )
    if expansion_potentials is not None:
