...
def plot(
    ...
    if solution is None:
        ...
        if visualize_timew:
            for (x, y), (l, u) in zip(coords, timew):
                ...
                timew_rects.append(
                    Rectangle(
                        (x, y + tw_height * l / max_tw),
                        tw_width,
                        tw_height * (u - l) / max_tw,
                    )
                )
        ...
        if visualize_nodes:
            ...
            # color=cmap(0)
            ax1.plot(
                xs, ys, "o", mfc="black", markersize=markersize, markeredgewidth=0.0
            )
        ax1.set_title(
            "{}, min {:.2f} routes".format(title, min_routes)
            if demand is not None
            else title
        )
    else:
        ...
        if depot is not None:
            routes = [
                r[r != 0]
                for r in np.split(tour, np.where(tour == 0)[0])
                if (r != 0).any()
            ]
        else:
            routes = [tour]
        ...
        for veh_number, r in enumerate(routes):
            color = cmap(
                len(routes) - veh_number
            )  # if not no_legend else 'black'  # Invert to have in rainbow order
            ...
            if visualize_nodes:
                ...
                ax1.plot(
                    ...
                    ys,
                    "o",
                    mfc=color if len(routes) > 1 else "black",
            ...
            if visualize_demands:
                ...
                for (x, y), d in zip(route_coords, route_demands):
                    ...
                    used_rects.append(
                        Rectangle(
                            (x, y), d_width, d_height * total_route_demand / capacity
                        )
                    )
                    dem_rects.append(
                        Rectangle(
                            (x, y + d_height * cum_demand / capacity),
                            d_width,
                            d_height * d / capacity,
                        )
                    )
            ...
            if timew is not None:
                ...
                for (x, y), n in zip(route_coords, r):
                    ...
                    assert (
                        t <= u
                    ), f"Time window violated for node {n}: {t} is not in ({l, u})"
                    ...
                    if visualize_timew:
                        ...
                        wait_rects.append(
                            Rectangle(
                                (x, y + tw_height * arr / max_tw),
                                tw_width,
                                tw_height * (t - arr) / max_tw,
                            )
                        )
            ...
            qvs.append(qv)
        title_makespan = (
            ", makespan {:.2f}".format(current_time) if timew is not None else ""
        )
        if demand is None:
            title = "{}, total distance {:.2f}{}".format(
                title, total_dist, title_makespan
            )
        else:
            title = "{}, {} routes (min {:.2f}), total distance {:.2f}{}".format(
                title, len(routes), min_routes, total_dist, title_makespan
            )
        ax1.set_title(title)
    ...
    if visualize_demands:
        ...
        if dem_rects is not None:
            ax1.add_collection(
                PatchCollection(
                    dem_rects, facecolor="black", alpha=1.0, edgecolor="black"
                )
            )
    ...
    if visualize_timew:
        if span_rects is not None:
            ax1.add_collection(
                PatchCollection(
                    span_rects, facecolor="whitesmoke", alpha=1.0, edgecolor="lightgray"
                )
            )
        if timew_rects is not None:
            ax1.add_collection(
                PatchCollection(
                    timew_rects, facecolor="lightgray", alpha=1.0, edgecolor="lightgray"
                )
            )
        if wait_rects is not None:
            ax1.add_collection(
                PatchCollection(
                    wait_rects, facecolor="black", alpha=1.0, edgecolor="black"
                )
            )
...
def plot_heatmap(ax1, coords, heatmap, mask, symmetric=True):
    if mask is not None:
        ...
        edge_colors = np.concatenate(
            (np.tile([1, 0, 0], (len(weights), 1)), weights[:, None]), -1
        )

        lc_edges = LineCollection(
            edges_coords, colors=edge_colors, linewidths=1, zorder=1
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--subplots", type=str, default="1x1", help="3x2 to plot 3 rows and 2 cols"
    )
    ...
    parser.add_argument(
        "-f", action="store_true", help="Force overwrite existing results"
    )
    ...
    def make_subplots():
        fig, axarr = plt.subplots(
            ...
            cols,
            squeeze=False,
            figsize=(args.figscale * rows, args.figscale * cols),
        )
        subplot_idx = 0
    ...
    start = args.offset
        ...
        len(instances),
        args.offset + args.num_visualize
    ...
    for i in range(start, end):
        ...
        if args.problem == "tsp":
            ...
            computed_cost = plot(
                ...
                title=f"Instance {i}",
                grid_size=grid_size,
                visualize_demands=args.visualize_demands,
                visualize_timew=args.visualize_timew,
                no_legend=args.no_legend,
            )
        ...
        if solution is not None:
            assert np.allclose(
                cost, computed_cost
            ), "Difference between saved cost {} and computed cost {} of solution! Are you using the right data/solutions?".format(
                cost, computed_cost
            )
